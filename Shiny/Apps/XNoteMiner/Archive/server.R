...
SERVER <- function(input, output) {
    ...
    df3 <- reactive({
        ...
        if (input$chk_add_pattern_matches) {
            ...
            df <- mutate(df, pattern_match = "")

            if (!is.null(final_regex())) {
                df <- mutate(df,
                             pattern_match = str_extract_all(
                                 input$select_text,
                                 regex( pattern = final_regex(),
                                        ignore_case = input$chk_ignore_case)) %>%
                                 lapply(., paste, collapse = ", ") %>% as.character()
                    ) %>%
                    select(pattern_match, everything())
            }
        }
